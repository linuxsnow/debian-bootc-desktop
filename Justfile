image_name := env("BUILD_IMAGE_NAME", "snow-gnome")
...
generate-bootable-image $base_dir=base_dir $filesystem=filesystem:
    #!/usr/bin/env bash
    if [ ! -e "${base_dir}/snowgnome.img" ] ; then
        fallocate -l 20G "${base_dir}/snowgnome.img"
    fi
    just bootc install to-disk --composefs-backend --via-loopback /data/snowgnome.img --filesystem "${filesystem}" --wipe --bootloader systemd

launch-incus:
    #!/usr/bin/env bash
    image_file=/tmp/snowgnome.img
